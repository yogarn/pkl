...
Arsitektur WLAN \textit{enterprise} terdiri atas beberapa komponen yang
saling terhubung untuk menyediakan akses jaringan nirkabel dan mendukung
pengelolaan jaringan secara terpusat. Komponen tersebut umumnya meliputi
\textit{access point} (AP), \textit{wireless LAN controller} (WLC), serta
infrastruktur jaringan yang menghubungkan AP dan controller dengan jaringan
lainnya.

Pada arsitektur berbasis controller, AP berfungsi menyediakan akses jaringan
nirkabel bagi perangkat pengguna, sedangkan controller menyediakan fungsi
pengelolaan dan koordinasi terhadap beberapa AP. Controller dapat digunakan
untuk mengelola konfigurasi WLAN, menerapkan kebijakan jaringan, serta
mengatur parameter operasional AP secara terpusat \parencite{DEZFOULI_REVIEW_2019}.
Infrastruktur switch menyediakan konektivitas antara AP, controller, dan
jaringan lainnya sehingga komponen-komponen tersebut dapat berkomunikasi
sebagai satu kesatuan jaringan.

Dalam arsitektur WLAN berbasis \textit{controller}, fungsi kontrol dan
penerusan trafik pengguna dapat dipisahkan. Pada arsitektur RUCKUS
SmartZone, pemisahan tersebut dilakukan antara fungsi
\textit{control and management plane} pada vSZ dan fungsi
\textit{data plane}. Trafik data pengguna dapat diteruskan melalui
jaringan lokal menggunakan mekanisme \textit{local breakout} atau melalui
\textit{tunneling} menuju \textit{data plane}, bergantung pada arsitektur
yang digunakan \parencite{RUCKUS_PRODUCT_GUIDE_2018}.
